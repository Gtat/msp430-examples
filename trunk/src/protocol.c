 ...
 *
 * @param [uint16_t t] DATA: seconds counter associated with the dynamic
 *                           bias switching, to determine if the electrode
 *                           voltage has settled
 * @param [struct flash_record * r] STORED: the memory region in flash to use for 
 *                                          reading data.
 * @param [char * toReTX] RETRY: the data to retransmit
 * @returns flag bits indicating data (e.g. alarm) conditions
 */
  ...
  (union mcu_to_pc * const p, enum mcu_id id, ...)
{
  ...
  switch (id)
  {
    ...
    case OK:
    {
      /* indicate which channels are in use */
    ...
    case RETRY:
    {
      ...
      toReTX = va_arg(ap, char *);
      for (index = 0; index < sizeof(union pc_to_mcu); ++index)
      {
        p->command.payload.samples[index] = toReTX[index];
      }
 ...
 *
 * @param p  location of packet buffer
 */
  ...
  (union pc_to_mcu * const p)
{
  ...
  switch (p->command.id)
  {
    ...
    {
#ifdef CONFIG_ENABLE_DYNAMIC_BIASING
      /* the packet's toggle bit indicates that we should set
       * the HI (i.e. second) voltage value
       */
      if (p->command.toggle)
      {
 ...
 * @returns the computed 8-bit CRC
 */
uint8_t crc8
...
#ifdef CONFIG_ENABLE_STORAGE_MODE
/**
 * Extract the contents of an uplink packet and store them in flash.
 *
 * @param   r      the region of flash to use
 * @param   p      the DATA packet to save
 * @param   flags  extra flags associated with this data point
 * @returns the number of bytes written to flash
 */
size_t store_packet
